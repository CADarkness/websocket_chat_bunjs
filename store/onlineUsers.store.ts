...
export default class OnlineUsers {
    ...
    static eject(id: string, callback?: (user: online_user) => void) {
        const existed = this.list.find(u => u.userId === id || u.connection === id)
        
        if (existed) {
            this.list = this.list.filter(user => user.userId !== id || user.connection !== id)

            console.log(`success eject online user: ${id}`)
